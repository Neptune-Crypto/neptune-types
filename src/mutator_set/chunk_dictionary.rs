...
impl ChunkDictionary {
    ...
    pub fn indices_and_leafs(&self) -> Vec<(ChunkIndex, Digest)> {
        self.dictionary.iter().map(|(k, (_mp, ch))| (*k, Tip5::hash(ch))).collect_vec()
    }
    pub fn indices_and_chunks(&self) -> Vec<(ChunkIndex, Chunk)> {
        self.dictionary.iter().map(|(k, (_mp, ch))| (*k, ch.clone())).collect_vec()
    }
    ...
    pub fn authentication_paths(&self) -> Vec<MmrMembershipProof> {
        self.dictionary.iter().map(|(_, (mp, _))| mp.to_owned()).collect()
    }
    ...
    pub fn contains_key(&self, key: &ChunkIndex) -> bool {
        self.dictionary.iter().any(|(chunk_index, _)| *chunk_index == *key)
    }
    ...
    pub fn all<F: FnMut(&(ChunkIndex, AuthenticatedChunk)) -> bool>(
        &self,
        f: F,
    ) -> bool {
    ...
    pub fn insert(
        ...
        if let Some((_found_chunk_index, found_authenticated_chunk)) = self
            .dictionary
            .iter_mut()
            .find(|(k, _v)| *k == index)
        {
    ...
    pub fn get_mut(&mut self, index: &ChunkIndex) -> Option<&mut AuthenticatedChunk> {
        self.dictionary.iter_mut().find(|(k, _v)| *k == *index).map(|(_k, v)| v)
    }
    ...
    pub fn remove(&mut self, index: &ChunkIndex) -> Option<AuthenticatedChunk> {
        ...
        if let Some(definite_position) = maybe_position {
            let (_chunk_index, authenticated_chunk) = self
                .dictionary
                .remove(definite_position);
...
pub mod tests {
    ...
    prop_compose! {
        pub fn propcompose_chunkdict() (dictionary in collection::vec((any::< u64 > (),
        collection::vec(proptest_arbitrary_interop::arb::< Digest > (), 0..6),
        collection::vec(any::< u32 > (), 0..17),), 37)) -> ChunkDictionary {
        ChunkDictionary::new(dictionary.into_iter().map(| (key, authpath, chunk) | (key,
        (MmrMembershipProof::new(authpath), Chunk { relative_indices : chunk, },)))
        .collect_vec()) }
    }
    ...
    async fn hash_test() {
        ...
        let chunkdict2 = ChunkDictionary::new(
            vec![(key1, value1.clone()), (key2, value2.clone())],
        );
        let key3: u64 = 89;
        let chunkdict3 = ChunkDictionary::new(
            vec![(key1, value1.clone()), (key2, value2.clone()), (key3, value2.clone()),],
        );
        ...
        let chunkdict3_alt = ChunkDictionary::new(
            vec![(key3, value2.clone()), (key1, value1.clone()), (key2, value2.clone()),],
        );
        assert_eq!(Hash::hash(& chunkdict3), Hash::hash(& chunkdict3_alt));
        let chunkdict3_switched = ChunkDictionary::new(
            vec![(key1, value2.clone()), (key2, value1), (key3, value2)],
        );
    ...
    async fn serialization_test() {
        ...
        let s_back_non_empty = serde_json::from_str::<ChunkDictionary>(&json_non_empty)
            .unwrap();
